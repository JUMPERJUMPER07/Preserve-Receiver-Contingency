...
import {
  Lock, User, ArrowRight, ShieldCheck, AlertCircle,
  FileBadge, CreditCard, Key, Check,
} from "lucide-react";
import { Logo } from "./Logo";
import { authenticateUser, registerUser, userExists } from "../users";

interface LoginProps {
  onLogin: (user: string, drt: string) => void;
}
...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        if (user) {
          onLogin(user.username.toUpperCase(), user.drt);
        } else {
        ...
        if (firstAccessStep === "validation") {
          ...
          if (!drtRegex.test(drt)) {
            setError("Formato do DRT inválido. Use números ou Números/UF (ex: 12345/SP).");
            return;
          }
          ...
          const user = await registerUser(drt, newPassword);
          onLogin(user.username.toUpperCase(), user.drt);
        }
      }
    } catch (err) {
  ...
  const accentFocus   = mode === "login" ? "focus:border-cyan-500/50"    : "focus:border-emerald-500/50";
  const accentRing    = mode === "login" ? "focus:ring-cyan-500/20"      : "focus:ring-emerald-500/20";

  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4 relative overflow-hidden select-none">
      {/* Background */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:50px_50px]" />
        <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-cyan-900/10 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-[-20%] right-[-20%] w-[60%] h-[60%] bg-blue-900/10 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: "1s" }} />
      </div>
      ...
      <div className="w-full max-w-[400px] bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl relative animate-in zoom-in-95 duration-500 ring-1 ring-white/5 flex flex-col overflow-hidden">
        {/* Accent top bar */}
        <div className={`h-1 w-full transition-all duration-700 bg-gradient-to-r ${mode === "login" ? "from-cyan-600 via-blue-500 to-indigo-600" : "from-emerald-600 via-teal-500 to-cyan-600"}`} />
        ...
        <div className="p-8 pb-10">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight text-center">
              Preserve <span className={`transition-colors duration-500 ${accentText}`}>Receiver</span>
            </h1>
          ...
          <div className="flex bg-slate-950/80 p-1 rounded-xl mb-8 border border-slate-800 relative">
            ...
            <button type="button" onClick={() => toggleMode("first_access")}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {mode === "login" ? (
              <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
                <InputGroup label="Usuário" icon={<User size={16} />} value={username}
                  onChange={e => setUsername(e.target.value)} placeholder="ID de Usuário"
                  accentClass="group-focus-within:text-cyan-400" borderClass={accentFocus} ringClass={accentRing} autoFocus />
                <InputGroup label="Senha" icon={<Lock size={16} />} type="password" value={password}
                  onChange={e => setPassword(e.target.value)} placeholder="••••••••"
                  accentClass="group-focus-within:text-cyan-400" borderClass={accentFocus} ringClass={accentRing} />
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                {firstAccessStep === "validation" ? (
                  <>
                    ...
                    <InputGroup label="DRT / Registro" icon={<FileBadge size={16} />} value={drt}
                      onChange={e => setDrt(e.target.value.toUpperCase())} placeholder="Ex: 12345/SP"
                      accentClass="group-focus-within:text-emerald-400" borderClass="focus:border-emerald-500/50" ringClass="focus:ring-emerald-500/20" autoFocus />
                    <InputGroup label="CPF" icon={<CreditCard size={16} />} value={cpf}
                      onChange={handleCpfChange} placeholder="000.000.000-00"
                      accentClass="group-focus-within:text-emerald-400" borderClass="focus:border-emerald-500/50" ringClass="focus:ring-emerald-500/20" />
                  </>
                ) : (
                  <>
                    <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-3 mb-2">
                      <p className="text-[10px] text-emerald-400/80 leading-relaxed font-mono">
                        {">"} Validação OK. DRT: <span className="text-emerald-300 font-bold">{drt}</span><br />
                        {">"} Defina sua credencial de acesso.
                      </p>
                    </div>

                    {/* Username (DRT, readonly) */}
                    <div className="space-y-1.5 opacity-60">
                      <label className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider ml-1">Usuário de Acesso</label>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-emerald-500"><User size={16} /></div>
                        <input type="text" value={drt} disabled
                          className="w-full bg-slate-900 border border-slate-800 text-slate-400 text-sm rounded-lg block pl-10 p-3 font-bold cursor-not-allowed font-mono" />
                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-emerald-500"><Check size={14} /></div>
                      </div>
                    </div>

                    <InputGroup label="Nova Senha" icon={<Key size={16} />} type="password" value={newPassword}
                      onChange={e => setNewPassword(e.target.value)} placeholder="Mínimo 6 caracteres"
                      accentClass="group-focus-within:text-emerald-400" borderClass="focus:border-emerald-500/50" ringClass="focus:ring-emerald-500/20" autoFocus />
                    <InputGroup label="Confirmar Senha" icon={<Check size={16} />} type="password" value={confirmPassword}
              ...
              {isLoading ? (
                <>
                  ...
                  <span className="tracking-wide text-xs uppercase">
                    {mode === "login" ? "Autenticando..." : firstAccessStep === "validation" ? "Validando..." : "Criando Conta..."}
                  </span>
                </>
              ) : (
                <>
                  <span className="tracking-wide text-xs uppercase">
                    {mode === "login" ? "Entrar no Sistema" : firstAccessStep === "validation" ? "Validar Credenciais" : "Criar Conta"}
                  </span>
                  <ArrowRight size={16} />
                </>
              )}
        ...
        <div className="bg-slate-950/90 p-2.5 text-center border-t border-slate-800 backdrop-blur-md">
          <p className="text-[9px] text-slate-600 font-mono tracking-wide">
            SYS.ID: PRC-RECEIVER-2.5.0 · BUILD: 20260426
          </p>
        </div>
      </div>
    </div>
...
interface InputGroupProps {
  ...
  accentClass: string; borderClass: string; ringClass: string;
  autoFocus?: boolean;
}

const InputGroup: React.FC<InputGroupProps> = ({
  label, icon, value, onChange, type = "text",
  placeholder, accentClass, borderClass, ringClass, autoFocus,
}) => (
  <div className="space-y-1.5 group">
    ...
    <div className="relative">
      ...
      <input type={type} value={value} onChange={onChange}
        className={`w-full bg-slate-950/50 border border-slate-700 text-slate-200 text-sm rounded-lg block pl-10 p-3 placeholder-slate-600 shadow-inner transition-all duration-300 ease-out outline-none focus:ring-2 ${borderClass} ${ringClass} focus:bg-slate-900/80`}
