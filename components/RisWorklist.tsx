...
import { Calendar, CheckCircle2, UploadCloud, Search, Database, ArrowUp, ArrowDown, ArrowUpDown } from 'lucide-react';
...
export const RisWorklist: React.FC<RisWorklistProps> = ({
  ...
  const sortedWorklist = [...filteredWorklist].sort((a, b) => {
    let comparison = 0;
    switch (sortField) {
      case 'patientName':    comparison = a.patientName.localeCompare(b.patientName);     break;
      case 'scheduledTime':  comparison = a.scheduledTime.localeCompare(b.scheduledTime); break;
      case 'modality':       comparison = a.modality.localeCompare(b.modality);           break;
    }
  ...
  const allModalities = [...new Set(worklist.map(i => i.modality).filter(Boolean))].sort();
  const completedCount = worklist.filter(i => i.status === 'completed').length;
    ...
    <div className="flex flex-col h-full bg-slate-900/0 relative">
      ...
      {draggedStudy && (
        <div className="absolute top-2 right-2 z-20 pointer-events-none animate-in fade-in zoom-in duration-300">
          <div className="bg-indigo-600/90 backdrop-blur border border-indigo-400/50 text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-lg shadow-indigo-500/20 flex items-center gap-2 font-mono">
      ...
      <div className="p-3 border-b border-white/5 flex flex-col gap-2 bg-white/5">
        <div className="flex gap-2">
          <div className="relative flex-1 group">
            ...
            <input
              type="text"
              placeholder="Buscar Paciente, Procedimento, ID..."
              value={searchTerm}
        ...
        <div className="flex gap-1 items-center bg-black/20 border border-white/10 rounded-full px-3 py-1 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
      ...
      <div className="sticky top-0 z-10 grid grid-cols-12 gap-2 px-3 py-2 bg-slate-950/80 backdrop-blur-md border-b border-white/5 text-[10px] font-bold text-slate-400 uppercase tracking-wider select-none shadow-sm">
        <div onClick={() => handleSort('scheduledTime')} className="col-span-1 flex items-center gap-1 cursor-pointer hover:text-indigo-400 transition-colors group">
          Hora <SortIcon field="scheduledTime" />
        </div>
        <div className="col-span-2 text-center">Data</div>
        <div onClick={() => handleSort('modality')} className="col-span-1 flex items-center gap-1 cursor-pointer hover:text-indigo-400 transition-colors group">
          Mod <SortIcon field="modality" />
        </div>
        <div onClick={() => handleSort('patientName')} className="col-span-4 flex items-center gap-1 cursor-pointer hover:text-indigo-400 transition-colors group">
          Paciente / ID <SortIcon field="patientName" />
        </div>
                  ...
                  onDrop={(e) => {
                    ...
                    if (draggedStudy) {
                      ...
                      try {
                        ...
                        if (data) onDropStudy(JSON.parse(data) as DicomStudy, item);
                      } catch { /* ignore */ }
                    }
                  }}
                  className={`
                    ...
                    ${draggedStudy && isMatch ? 'bg-emerald-900/40 border border-emerald-500/50 shadow-lg shadow-emerald-500/20 z-10' : ''}
                    ${draggedStudy && !isMatch ? 'opacity-40 grayscale-[0.8]' : ''}
                  `}
                >
                  <div className="col-span-1 text-slate-400 font-mono tabular-nums text-[10px]">
                    {timePart}
                  </div>
                  ...
                  <div className="col-span-3 text-slate-400 truncate opacity-80 text-[11px]" title={item.procedure}>
                    {item.procedure}
                  </div>
                  <div className="col-span-1 text-right">
                    ...
                    {item.status === 'completed' && (
                      <CheckCircle2 size={10} className="text-emerald-500 inline ml-1" />
                    )}
                  </div>
                </div>
              );
      ...
      <div className="p-1.5 border-t border-white/5 bg-slate-950/50 text-[9px] text-slate-500 text-center uppercase tracking-widest font-medium font-mono">
        {sortedWorklist.length} de {worklist.length} · {completedCount} vinculados · Solte aqui para vincular
      </div>
    </div>
  );
};
