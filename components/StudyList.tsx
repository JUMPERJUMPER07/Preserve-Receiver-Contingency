...
import { Search, Database, CheckCircle2, Eye, ArrowUp, ArrowDown, ArrowUpDown, GripVertical } from 'lucide-react';
...
export const StudyList: React.FC<StudyListProps> = ({ studies, onSelect, onDetails, selectedId, onDragStart }) => {
  ...
  const sortedStudies = [...filteredStudies].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'studyDate':   comparison = a.studyDate.localeCompare(b.studyDate);     break;
      case 'receivedAt':  comparison = a.receivedAt.localeCompare(b.receivedAt);   break;
    }
  ...
  const allModalities = [...new Set(studies.map(s => s.modality).filter(Boolean))].sort();
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/0">
      ...
      <div className="p-3 border-b border-white/5 flex flex-col gap-2 bg-white/5">
        <div className="flex gap-2">
          <div className="relative flex-1 group">
            ...
            <input
              type="text"
              placeholder="Buscar Paciente, Accession, ID..."
              value={searchTerm}
      ...
      <div className="sticky top-0 z-10 grid grid-cols-12 gap-2 px-3 py-2 bg-slate-950/80 backdrop-blur-md border-b border-white/5 text-[10px] font-bold text-slate-400 uppercase tracking-wider select-none shadow-sm">
        ...
        <div onClick={() => handleSort('receivedAt')} className="col-span-1 flex items-center gap-1 cursor-pointer hover:text-cyan-400 transition-colors group">
          Hora <SortIcon field="receivedAt" />
        </div>

        <div className="col-span-1 text-center">Mod</div>

        <div onClick={() => handleSort('studyDate')} className="col-span-2 flex items-center gap-1 cursor-pointer hover:text-cyan-400 transition-colors group">
          Data <SortIcon field="studyDate" />
        </div>

        <div onClick={() => handleSort('patientName')} className="col-span-3 flex items-center gap-1 cursor-pointer hover:text-cyan-400 transition-colors group">
          ...
          <div className="pb-2">
            {sortedStudies.map((study, index) => (
              ...
              >
                ...
                <div className="col-span-2 text-slate-400 flex items-center tabular-nums text-[10px]">
                  {study.studyDate}
                </div>
                ...
                <div className="col-span-3 text-slate-400 pr-1 min-w-0">
                  <div className="truncate opacity-90 text-[11px]" title={study.description}>
                    {study.description || '-'}
                  </div>
      ...
      <div className="p-1.5 border-t border-white/5 bg-slate-950/50 text-[9px] text-slate-500 text-center uppercase tracking-widest font-medium font-mono">
        {sortedStudies.length} de {studies.length} estudos · Arraste para vincular
      </div>
    </div>
  );
};
